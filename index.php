...
try {
    ...
    // Initialize SQLite database with checks
    ...
    if ($method === 'GET') {
        ...
        // Add other GET routes here if needed
    }
    ...
    if ($method === 'POST') {
        if ($request_uri === '/send-magic-link') {
            ...
            if ($stmt->execute([$name, $email, $_SESSION['user_id']])) {
                ...
                log_message("Profile updated for user ID {$_SESSION['user_id']}: $name, $email");
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Failed to update profile']);
                log_message("Failed to update profile for user ID {$_SESSION['user_id']}", 'ERROR');
            }
            exit;
        ...
        // Add other POST routes here if needed
    }
